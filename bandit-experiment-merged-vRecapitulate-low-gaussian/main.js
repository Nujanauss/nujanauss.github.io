...
async function loadIndex() {
    ...
    const preferenceJSON = await loadAgent(files, 'preference');
...
function loadInstructions6() {
    ...
    document.getElementById('numberTurnsINSTR6_2').innerHTML = (settings.moves - 5);
...
async function loadPhase1(numberOfMoves, numberOfRounds, training, rewarding, ids) {
    ...
    function handleClick(square) {
      ...
      if (shouldReward) {
        ...
        reward = training ? 1 : chanceToWin[((phase1Round - 1) * numberOfMoves + currentRow)][x];
...
function loadPhase4(numberOfMoves, numberOfRounds, training, ids) {
    ...
    function handleClick(square) {
      ...
      if (shouldReward) {
        ...
        square.classList.add(`reward${x}-clicked`);
        scoreDisp.classList.remove('gone');
        reward = training ? 1 : chanceToWin[((phase1Round + phase2Round + phase3Round + phase4Round - 1) * numberOfMoves + currentRow)][x];
        scoreDisp.innerHTML = reward;
        score = score + reward;
        scoreText.textContent = 'Score: ' + score;
      } else {
...
function setInFocus(subrow, bigSize) {
  const squares = subrow.querySelectorAll('.square-finder');
  squares.forEach(sq => {
